...
export default function ReelsSlider() {
  // Properly typed video refs
  const videoRefs = useRef<Array<HTMLVideoElement | null>>([]);

  // Explicit type for currentIndex
  ...
  // Framer Motion variants with proper Transition typing
  const reelVariants: Variants = {
    ...
    visible: {
      ...
      transition: {
        duration: 0.8,
        ease: [0.42, 0, 0.58, 1], // cubic-bezier array instead of string
      } as Transition,
    },
  };
    ...
    <div className="w-full py-12 m-auto">
      ...
      >
        {reelsData.slice(0, 5).map((reel, index) => (
          <SwiperSlide key={reel.id} className="flex justify-center items-center">
            <motion.div
              ...
              <div className="rounded-xl transition-transform duration-300 hover:shadow-[0_0_30px_#fff]">
                <video
                  ref={(el: HTMLVideoElement | null) => {
                    videoRefs.current[index] = el; // must return void
                  }}
